...
protocol PhotoListService {
    func getPhotoList(text: String) -> AnyPublisher<PhotoObject, NetworkingError>
}
...
class PhotoListServiceImpl: PhotoListService, ObservableObject {
    ...
    func getPhotoList(text: String = "Yorkshire") -> AnyPublisher<PhotoObject, NetworkingError> {
        
        guard let url = URL(string: "\(BaseUrl.baseUrl) /?method=flickr.photos.search&api_key=\(APIKey.apiKey)&text=\(text)&format=json&nojsoncallback=1") else {
